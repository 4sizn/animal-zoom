...
- 지원할 아바타 종류/스펙: `cat | bear | penguin | rabbit` (MVP 데모 기준, 추가 타입은 이후 확장)
- 방 테마 수: `3` (MVP: `library`, `cafe`, `default`)
- 최소/최대 그래픽 품질 목표: `MVP는 데스크톱 WebGL2 기준 "중간"(post-process 없음, 저비용 라이트/머티리얼)로 고정`

### B) 참가자 규모 및 성능 예산

- 단계별 목표 참가자 수 (MVP / Beta / GA): `MVP 2~4 / Beta 8 / GA 12` (이 문서는 MVP 구현을 우선)
- 디바이스 클래스별 FPS 목표 (데스크톱 기준): `MVP: 데스크톱 30 FPS 이상 (권장 60), 모바일은 MVP 범위 밖`
- 메모리/CPU 예산 제약: `MVP: 4인 기준 브라우저 탭 1개에서 안정 동작 (명시적 수치는 추후 계측으로 확정)`

### C) 상호작용 및 UX

- 참가자 칸 클릭 시 동작 (스포트라이트/핀/없음): `MVP: 없음 (핀/스포트라이트는 추후 작업)`
- 지원할 이모트 종류: `wave | clap | thumbsup | laugh` (MVP 데모용)
- 접근성 요구사항 (키보드, 저모션, 대비 등): `MVP: 키보드 포커스 이동 가능 + prefers-reduced-motion 존중 (세부는 추후 정의)`

### D) 데이터 및 동기화 계약

- 씬 업데이트에 필요한 실시간 입력 필드: `MVP: avatarType, roomTheme, isSpeaking, isMuted (필요 시 emote)`
- 업데이트 주기/지연 허용치: `MVP: 10 Hz 내외 / 지연 200ms 내외 허용 (정확 수치는 추후 측정)`
- 지연 도착/역순 업데이트 충돌 처리 규칙: `MVP: last-write-wins (서버 수신 순서 기준)`

### E) 에셋 및 파이프라인

- 에셋 조달 전략 (직접 제작 / 마켓플레이스 / 혼합): `MVP: placeholder (primitive mesh + 단색/기본 텍스처), 이후 혼합으로 확장`
- 허용 포맷 및 에셋 단위 용량 제한: `MVP: glTF/glb, 단일 에셋 2MB 이하 권장 (추후 조정)`
- 라이선스/컴플라이언스 제약: `MVP: 상업 사용 가능/저작권 클린 에셋만 사용 (출처/라이선스 기록 필요)`

### F) 운영 제약

- 브라우저 지원 매트릭스 (필수/권장): `필수: Chrome(최신) / 권장: Edge, Safari (Firefox best-effort)`
- WebGL 미지원/로딩 실패 시 폴백 동작: `MVP: 2D 폴백 화면 + 안내 문구 (3D 비활성)`
- 텔레메트리/진단 수집 요구사항: `MVP: 없음 (콘솔 로그/에러 바운더리 수준), 추후 요구사항 정의`
