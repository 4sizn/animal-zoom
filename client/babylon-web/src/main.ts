...
function renderRoute(path: RoutePath): void {
  ...
  switch (path) {
    ...
    case "/room": {
      ...
      controls.append(addParticipantButton, removeParticipantButton, participantCount);
    ...
    case "/my-room": {
      ...
      controls.append(title, themeLabel, themeSelect, avatarLabel, avatarSelect);

      shell.append(controls, createCanvasPlaceholder("my-room-canvas", "my-room-canvas"));
...
function createCanvasPlaceholder(id: string, className: string): HTMLCanvasElement {
...
function getRouteCanvases(path: RoutePath): HTMLCanvasElement[] {
  switch (path) {
    ...
    case "/room": {
      return [...appRoot.querySelectorAll<HTMLCanvasElement>("#room-canvas-grid canvas.room-canvas")];
    }
    case "/my-room": {
      const canvas = appRoot.querySelector<HTMLCanvasElement>("#my-room-canvas");
      return canvas ? [canvas] : [];
    }
  }
}
...
function createEngineRuntime(): {
  ...
  const roomParticipantViews = new Map<HTMLCanvasElement, RoomParticipantView>();
  ...
  engine.runRenderLoop(() => {
    if (runtimeRoute === "/solo") {
      ...
      try {
        ...
        if (!hasRenderError) {
          hasRenderError = true;
          console.error("Solo scene render disabled after runtime error", error);
        }
      }
    ...
    if (runtimeRoute === "/my-room") {
      ...
      try {
        ...
        if (!hasMyRoomRenderError) {
          ...
          console.error("My-room scene render disabled after runtime error", error);
        }
      }
    ...
    try {
      ...
      if (!roomErroredParticipantIds.has(roomView.participantId)) {
        ...
        console.error(`Room scene render disabled for ${roomView.participantId}`, error);
      }
    }
  });
  ...
  const syncMyRoomSceneFromControls = (): void => {
    const themeSelect = appRoot.querySelector<HTMLSelectElement>("#my-room-theme-select");
    const avatarSelect = appRoot.querySelector<HTMLSelectElement>("#my-room-avatar-select");
  ...
  const updateRoomParticipantCount = (): void => {
    const countElement = appRoot.querySelector<HTMLElement>("#room-participant-count");
    if (!countElement) {
      return;
    }
  ...
  const updateParticipantViewEnabled = (canvas: HTMLCanvasElement, enabled: boolean): void => {
  ...
  const registerRoomControls = (): void => {
    ...
    const addButton = appRoot.querySelector<HTMLButtonElement>("#room-add-participant");
    const removeButton = appRoot.querySelector<HTMLButtonElement>("#room-remove-participant");
    ...
    const addParticipant = (): void => {
      ...
      const canvas = createCanvasPlaceholder(`room-canvas-${nextParticipantIndex}`, "room-canvas");
      ...
      try {
        ...
        canvas.remove();
        console.error(`Failed to initialize room scene for ${participantId}`, error);
      }
    };
  ...
  const registerViews = (path: RoutePath, canvases: HTMLCanvasElement[]): void => {
    runtimeRoute = path;
    ...
    if (path === "/solo") {
      ...
      try {
        soloSceneBundle = createSingleViewSceneBundle(engine, {
          attachControl: false,
          inputElement: soloCanvas,
        });
        engine.registerView(soloCanvas, soloSceneBundle.camera);
