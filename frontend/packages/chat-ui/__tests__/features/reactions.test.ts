...
describe('Message Reactions', () => {
  ...
  describe('Reaction Limits', () => {
    it('should enforce maximum reactions per message', () => {
      ...
      const { addReaction, MAX_REACTIONS_PER_MESSAGE } = useChatStore.getState();

      if (addReaction && MAX_REACTIONS_PER_MESSAGE) {
        const emojis = ['👍', '❤️', '🎉', '😂', '😍', '🔥', '✅', '💯', '⭐', '💪', '🚀'];

        // Add reactions up to limit
        for (let i = 0; i < Math.min(emojis.length, MAX_REACTIONS_PER_MESSAGE); i++) {
    ...
    it('should preserve reaction timestamps', () => {
      ...
      if (addReaction) {
        ...
        if (reaction?.timestamp) {
          ...
          expect(reactionTime.getTime()).toBeGreaterThanOrEqual(beforeTime.getTime());
          expect(reactionTime.getTime()).toBeLessThanOrEqual(afterTime.getTime());
        }
      }
    });
  });
