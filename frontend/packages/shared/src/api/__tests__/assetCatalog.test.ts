...
describe("Asset Catalog API", () => {
  ...
  describe("getAssetUrl", () => {
    it("should fetch presigned URL for asset", async () => {
      const mockUrl = {
        url: "https://s3.amazonaws.com/bucket/asset.glb?signature=xyz",
      };

      mockApiClient.get = mock(() => Promise.resolve({ data: mockUrl }));

      const result = await assetCatalogApi.getAssetUrl(
        "assets/avatar/humanoid/1.0.0/uuid-1.glb",
      );
