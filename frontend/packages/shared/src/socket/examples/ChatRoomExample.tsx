...
export function ChatRoomExample() {
  const [controller] = useState(() => new WebSocketClientController({ autoConnect: false }));
    ...
    <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '20px', padding: '10px', backgroundColor: '#f0f0f0' }}>
        ...
        {isInRoom && (
          <>
            {' | '}
            <strong>Room:</strong> {currentRoom} ({participantCount} participants)
          </>
        )}
      </div>
            ...
            {messages.length === 0 ? (
              ...
              messages.map((msg, index) => (
                <div key={index} style={{ marginBottom: '10px', padding: '5px' }}>
                  <strong style={{ color: '#0066cc' }}>{msg.senderName}:</strong>{' '}
      ...
      {/* Instructions */}
      <div style={{ marginTop: '40px', padding: '15px', backgroundColor: '#f9f9f9' }}>
        <h3>사용 방법</h3>
        <ol>
          <li>
        ...
        <p>
          <strong>Note:</strong> 이 예제를 실행하려면 WebSocket 서버가 실행 중이어야 하며,
          인증 토큰이 설정되어 있어야 합니다.
        </p>
      </div>
    </div>
