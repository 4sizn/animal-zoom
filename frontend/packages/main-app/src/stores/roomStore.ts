...
import type { Participant, Room } from "@animal-zoom/shared/types";
...
function participantToParticipantInfo(
    ...
    devtools(
      (set, get) => ({
        ...
        updateUserJoinState: (state) =>
          set((prev) => ({
            currentUser: prev.currentUser
              ? { ...prev.currentUser, joinState: state }
              : null,
          })),
        ...
        removeFromWaitingRoom: (participantId) =>
          set((prev) => ({
            waitingParticipants: prev.waitingParticipants.filter(
              (p) => p.id !== participantId,
            ),
          })),
