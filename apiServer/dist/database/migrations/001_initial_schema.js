...
async function up(db) {
    ...
    await db.schema
        ...
        .addColumn('createdAt', 'timestamp', (col) => col.notNull().defaultTo((0, kysely_1.sql) `now()`))
        .addColumn('updatedAt', 'timestamp', (col) => col.notNull().defaultTo((0, kysely_1.sql) `now()`))
        .execute();
    await db.schema
        .createTable('rooms')
        .addColumn('id', 'uuid', (col) => col.primaryKey().defaultTo((0, kysely_1.sql) `uuid_generate_v4()`))
...
async function down(db) {
    ...
    await (0, kysely_1.sql) `DROP EXTENSION IF EXISTS "uuid-ossp"`.execute(db);
}
